"""
evaluation/a3/evaluate_a3.py — A3 Evaluation (VQ-VAE)
====================================================
Evaluates A3 VQ-VAE + CameraEncoder on test split.

Outputs:
  results/a3/full_eval_a3.csv
  results/a3/summary_a3.txt
"""
...
from morph_config import CYCLES_DIR, SPLIT_FILE, BATCH_SIZE, A3_CKPT_DIR, A3_RESULTS_DIR
...
def main():
    ...
    vq_ckpt = Path(A3_CKPT_DIR) / 'stage1_vqvae_a3.pt'
    if not vq_ckpt.exists():
        print("Missing VQ-VAE checkpoint. Run training/a3/train_a3.py first."); return
    ...
    results_dir = Path(A3_RESULTS_DIR)
    results_dir.mkdir(parents=True, exist_ok=True)

    all_results = []

    for enc_name, in_ch in [('A', 1), ('B', 1), ('C', 2)]:
        ckpt_p = Path(A3_CKPT_DIR) / f'encoder_a3_{enc_name}.pt'
        if not ckpt_p.exists():
            print(f"  Skipping A3-{enc_name} — no checkpoint"); continue
        ...
        print(f"\nEvaluating A3-{enc_name}...")
        with torch.no_grad():
            for gt, g, rppg, sids in tqdm(test_loader, desc=f"A3-{enc_name}"):
                gt_np = gt.numpy()[:, 0, :]
                if enc_name == 'A':
                    x_in = g.to(device)
                elif enc_name == 'B':
                ...
                for i in range(len(gt_np)):
                    ...
                    all_results.append({
                        'architecture': 'A3', 'encoder': enc_name, 'sid': sid, 'dataset': ds,
                        'shape_r': r,
    ...
    df.to_csv(results_dir / 'full_eval_a3.csv', index=False)

    print("\n" + "=" * 60)
    print("A3 (VQ-VAE) EVALUATION RESULTS")
    print("=" * 60)
    ...
    for enc in df['encoder'].unique():
        ...
        for ds in sub['dataset'].unique():
            ds_subj = sub[sub['dataset'] == ds].groupby('sid')['shape_r'].mean()
            print(f"    {ds:12s}: r={ds_subj.mean():.4f} ({ds_subj.shape[0]} subjects)")

    with open(results_dir / 'summary_a3.txt', 'w') as f:
        f.write(df.to_string(index=False))
    print(f"\nSaved to {results_dir / 'full_eval_a3.csv'}")
