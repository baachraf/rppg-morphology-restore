"""
evaluation/a12/evaluate_a12.py — A12 VMD-Peak-Aligned Encoder Evaluation
=========================================================================
Reports:
  - Per-subject r (shape fidelity)    — compare to V5-B baseline 0.711
  - Cross-subject r (collapse metric) — goal: < 0.70  (V5-B: 0.808)
  - IPA error, H2/H1 error
  - Per-dataset and per-FPS breakdowns

Outputs:
  results/a12/full_eval_a12.csv
  results/a12/summary_a12.txt
"""
...
class A12EvalDataset(Dataset):
            ...
            for npz_f in sorted(d.glob('*_a12_cycles.npz')):
                try:
                    ...
                    for i in range(len(gt)):
                        self.items.append((vmd_g[i], gt[i], sid, ds_name))
    ...
    def __getitem__(self, idx):
        vmd_g, gt, sid, ds_name = self.items[idx]
...
def load_models(device):
    ...
    if not ckpt_path.exists():
        raise FileNotFoundError(f'A12 checkpoint not found: {ckpt_path}\n'
                                f'Run train_a12.py first.')
    ...
    print(f'Loaded A12 encoder (val_r={ckpt.get("val_r", "?"):.4f} @ epoch {ckpt.get("epoch","?")})')
    return vae, encoder
...
def main():
    ...
    test_ds = A12EvalDataset(CYCLES_DIRS, test_sids, split_df)
    if len(test_ds) == 0:
        print('ERROR: No A12 test cycles found.'); return
    ...
    for vmd_g, gt_batch, sids, ds_names in tqdm(test_loader):
        vmd_g = vmd_g.to(device)
        gt_np = gt_batch.numpy()

        with torch.no_grad():
            z_pred   = encoder(vmd_g)
            ppg_pred = vae.decode(z_pred).cpu().squeeze(1).numpy()
    ...
    df.to_csv(RESULTS_A12 / 'full_eval_a12.csv', index=False)
    ...
    fps60_mask = df['dataset'] == 'fps2023_60'
    fps30_mask = df['dataset'] == 'fps2023'
    r_fps60 = df[fps60_mask].groupby('sid')['shape_r'].mean().mean() \
              if fps60_mask.any() else float('nan')
    r_fps30 = df[fps30_mask].groupby('sid')['shape_r'].mean().mean() \
              if fps30_mask.any() else float('nan')
    ...
    lines = [
        '=' * 68,
        'A12 (VMD-Peak-Aligned 1ch Encoder) EVALUATION',
        '=' * 68,
        '',
    ...
    (RESULTS_A12 / 'summary_a12.txt').write_text('\n'.join(lines), encoding='utf-8')
    print(f'\nSaved to {RESULTS_A12}')
