"""
extraction/extract_rgb_windows.py — A6: Fixed-Time Raw RGB Window Extraction
=============================================================================
Extracts fixed-time windows of raw RGB traces from parsed CSVs, paired with
GT PPG cycle targets. NO rPPG extraction, NO cycle boundary detection.

For each subject:
  1. Load parsed CSV (per-frame R, G, B patch values + time_sec)
  2. Load GT PPG npz (ppg_values, ppg_times, ppg_hz)
  3. Align camera and GT timelines
  4. Extract GT PPG peaks → build average cycle template (target)
  5. Slide W-second windows over raw RGB with step S
  6. For each window: extract mean R, G, B → detrend + z-score → PCHIP to 256
  7. Save: (rgb_window, gt_template, sid, dataset, window_start_time)

Output per subject: {stem}_rgb_windows.npz in E:/.../data/{dataset}/rgb_windows/

GT PPG peaks are used ONLY for building the training target (average cycle).
They are NOT used for input segmentation — windows are fixed-time.
"""
...
def detrend_zscore(sig):
    t = np.arange(len(sig), dtype=np.float64)
    coeffs = np.polyfit(t, sig, 1)
    detrended = sig - np.polyval(coeffs, t)
    std = detrended.std()
    if std < 1e-8:
        return np.zeros_like(detrended, dtype=np.float32)
    return (detrended / std).astype(np.float32)
...
def process_subject(csv_path, ppg_npz_path, out_dir, ds_name, sid):
    ...
    for i in range(len(peaks) - 1):
        ...
        if min_cycle_sec <= dur <= max_cycle_sec:
            seg = pchip_resample(norm01(p_vals_act[peaks[i]:peaks[i + 1]]), RESAMPLE_LEN)
            gt_segs.append(seg)
    ...
    for wi in range(n_windows):
        ...
        # Check GT availability FIRST — only proceed if GT overlap is sufficient
        p_in_window = (p_times_act >= t_start) & (p_times_act <= t_end)
        if p_in_window.sum() <= p_hz * 0.5:
            continue

        r_seg = detrend_zscore(R_mean[idx])
        g_seg = detrend_zscore(G_mean[idx])
        b_seg = detrend_zscore(B_mean[idx])

        r_resampled = pchip_resample(r_seg, RESAMPLE_LEN).astype(np.float32)
        g_resampled = pchip_resample(g_seg, RESAMPLE_LEN).astype(np.float32)
        b_resampled = pchip_resample(b_seg, RESAMPLE_LEN).astype(np.float32)

        rgb_windows.append(np.stack([r_resampled, g_resampled, b_resampled]))
    ...
    assert len(rgb_windows) == len(gt_targets) == len(win_times), \
        f"Length mismatch: rgb={len(rgb_windows)}, gt={len(gt_targets)}, times={len(win_times)}"

    out_path = out_dir / (csv_path.stem + '_rgb_windows.npz')
    np.savez_compressed(
        out_path,
...
def main():
    print(f'\nextract_rgb_windows.py — A6 Fixed-Time Raw RGB Window Extraction')
    print(f'  Window: {WINDOW_SEC}s | Step: {STEP_SEC}s | Resample: {RESAMPLE_LEN}')
    print()
    ...
    for ds_name, csv_dir, ppg_dir in DATASETS:
        ...
        out_dir = csv_dir.parent / 'rgb_windows'
